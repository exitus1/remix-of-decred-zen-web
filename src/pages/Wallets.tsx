            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {desktopWallets.map((wallet) => (
                <Card key={wallet.name} className="hover:shadow-lg transition-shadow relative">
                  {wallet.name === "Decrediton" && (
                    <div className="absolute top-3 right-3 flex flex-col gap-1">
                      <Badge variant="outline" className="border-accent text-accent text-xs px-2 py-0.5">
                        Trezor
                      </Badge>
                      <Badge variant="outline" className="border-accent text-accent text-xs px-2 py-0.5">
                        Ledger
                ...
                <Card key={wallet.name} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-4">
                        {wallet.logo && (
                          <div className={`w-16 h-16 rounded-lg bg-background flex items-center justify-center ${wallet.name === 'Exodus' ? 'p-0' : 'p-2'}`}>
                            <img 
                              src={wallet.logo} 
                              alt={wallet.name}
                              className="w-full h-full object-contain"
